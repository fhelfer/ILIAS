...
use ILIAS\Rating\Category\RatingCategoryInputForm;
...
class ilRatingGUI
{
    ...
    public function getHTML(
        ...
        $input = $this->ui_factory->input()->field()->rating('label')->withValue(5)->withAdditionalOnLoadCode(function ($id): string {
            return '';
        });
    ...
    public function getBlockHTML(string $a_title): string
    {
        ...
        $form = (new RatingCategoryInputForm(
            $this->ui_factory,
            $this->ctrl_path,
            $this->ctrl,
            $categories,
        ...
        $panel = $ui->factory()->panel()->secondary()->legacy(
            $a_title,
            $this->ui_factory->legacy()->content($this->ui_renderer->render($form))
        );
